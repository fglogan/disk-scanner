...
use disk_bloat_scanner_lib::{
    models::CleanupReq,
    utils::{cleanup::validate_deletion_request, path::validate_scan_path},
};
...
fn test_validate_scan_path_blocks_all_system_directories() {
    ...
    for dir in system_dirs {
        if std::path::Path::new(dir).exists() {
            ...
            assert!(
                ...
                "Should contain security message for {}: {}",
                dir,
                err_msg
            );
        }
    ...
    for pattern in blocked_patterns {
        ...
        assert!(
            result.is_err(),
            "Should block pattern directory: {}",
            pattern
        );
...
fn test_validate_scan_path_allows_safe_directories() {
    ...
    for dir in safe_dirs {
        ...
        if let Err(err_msg) = result {
            ...
            assert!(
                !err_msg.contains("protected system directory"),
                "Should not block safe directory {}: {}",
                dir,
                err_msg
            );
        }
    ...
    if let Ok(home) = std::env::var("HOME") {
        ...
        if let Err(err_msg) = result {
            assert!(
                !err_msg.contains("protected system directory"),
                "Should not block home directory {}: {}",
                home,
                err_msg
            );
        }
...
fn test_comprehensive_security_coverage() {
    ...
    for path in &critical_paths {
        ...
        assert!(
            del_result.is_err(),
            "Deletion validation should block: {}",
            path
        );
    }
}
